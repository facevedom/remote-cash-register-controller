...
def initDB():
    DATABASE_PATH = "{}/cash_register.db".format(
...
def canCloseCashRegister(userId, cash_register):
    ...
    date = currentTime[0]
    time = '%s:%s' % (currentTime[1].split(
        ':')[0], currentTime[1].split(':')[1])
    connection, sql_service = initDB()
    sql_service.execute(ReservationTableQueries.CAN_CLOSE_CASH_REGISTER, [
                        cash_register, date, time, time])
    canClose = False
    ...
    if message:
        ...
        if action == CashActions.CLOSE.value:
            ...
            if canClose:
                ...
                t = threading.Thread(target=actuator.request_closing, args=[
                                     cash_register])
                t.start()
            else:
                if userReserved:
...
def getAvailability(cash_register, date, initial_hour, finish_hour, sql_service):
    sql_service.execute(ReservationTableQueries.EXISTING_RESERVATION, [
                        cash_register, date, initial_hour, initial_hour, finish_hour, finish_hour])
    availability = True
...
def doReservation(message, sql_service, userId, channel):
    ...
    available, userReserved = getAvailability(
        cash_register, date, initial_hour, finish_hour, sql_service)
    if available:
        query = ReservationTableQueries.INSERT % (
            userId, cash_register, date, initial_hour, finish_hour)
        sql_service.execute(query)
        reserved = ResponseSlack.CASH_REGISTER_RESERVED % (
            userId, cash_register)
        messagePayload = ResponseSlack.GENERIC % (reserved, channel)
    else:
        availability = ResponseSlack.CASH_REGISTER_NOT_AVAILABLE % (
            userId, cash_register, userReserved)
        messagePayload = ResponseSlack.GENERIC % (availability, channel)
    return messagePayload


def getReservations(sql_service, cash_register, date, channel):
    sql_service.execute(ReservationTableQueries.GET_RESERVATIONS, [
                        cash_register, date])
    ...
    for register in data:
        ...
        text = '{"type": "plain_text","text": "%s %s from %s to %s","emoji": true}' % (
            register[ReservationTable.CASH_REGISTER.value], register[ReservationTable.DATE.value], register[ReservationTable.INITIAL_HOUR.value], register[ReservationTable.FINISH_HOUR.value])
        textCustom = {
            "type": "section",
...
def ResponseSlackBot(messagePayload):
    ...
    r = requests.post(
        url='https://slack.com/api/chat.postMessage', data=payload, headers=head)
...
def processRequest(text, clientId, userId, channel):
    ...
    if message:
        ...
        if action == CashActions.RESERVE.value:
            messagePayload = doReservation(
                message, sql_service, userId, channel)
        elif action == CashActions.RESERVATIONS.value:
            cash_register = message[SlackMessage.CASH_REGISTER.value]
            date = message[SlackMessage.DATE.value]
            messagePayload = getReservations(
                sql_service, cash_register, date, channel)
        else:
            messagePayload = ResponseSlack.INVALID_ACTION % channel
    else:
        messagePayload = ResponseSlack.GENERIC % (
            ResponseSlack.UNKNOWN, channel)
...
def chat_bot():
    ...
    if process and request.get_json().get('event').get('subtype') != 'bot_message' and request.get_json().get('event').get('subtype') != 'message_deleted':
        ...
        t = threading.Thread(target=processRequest(
            text, clientId, userId, channel))
        t.start()
    return 'ok', 200


def date():
    print(json.loads(request.form.get('payload'))
          ['actions'][0]['selected_date'])
