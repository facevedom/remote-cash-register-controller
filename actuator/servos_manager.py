...
def lock(lock_file):
    if os.path.isfile(lock_file):
        ...
        time.sleep(3) # TODO implement actual remediation processes in this situation
        unlock(lock_file)
    else:
        try:
            os.mknod(lock_file)
...
def close_register(register_id):
    ...
    try:
        ...
        SERVO_INITIAL_POSITION = int(config[register_id]['SERVO_INITIAL_POSITION'])
        SERVO_MIDDLE_POSITION = int(config[register_id]['SERVO_MIDDLE_POSITION'])
        logging.debug("Configuration read from '%s'", config_file_path)
    except Exception:
        logging.exception("Error when reading properties from '%s'", config_file_path)
        raise
